...
private:
    ...
    int IsHeightBalanced(Node *cur){
    if(cur == nullptr) return 1;
    if(cur != nullptr) {
           if((IsHeightBalanced(cur->left) &&
            IsHeightBalanced(cur->right) &&
            abs(height(cur->left) - height(cur->right)) <= 1)) return 1;
    }
    return 0;
    }
